...
document.addEventListener('DOMContentLoaded', function() {
    console.log('El documento HTML (Carrito) ha sido completamente cargado y analizado');
...
function ModificarCantidadAComprar(idProducto, nuevaCantidad) {
    const retrievedJsonString = sessionStorage.getItem('listaCarrito');
    const listaAux = JSON.parse(retrievedJsonString); // Convierto la cadena JSON de nuevo a un objeto JavaScript
...
function handleDeleteButtonClick(event) {
    ...
    cardElement.remove(); // Elimino la tarjeta del DOM
    MostrarPrecioBrutoProductos();
    ActualizarPrecioFinal();
}
        ...
        if (radio.checked) {
            switch (radio.id) {
                case "option1": // Domicilio
                    ...
                    break;
                case "option2": // Sucursal correo argentino
                    precioEnvioCalculado = 3000;
                    break;
                case "option3": // Nuestro Local
                    precioEnvioCalculado = 0;
                    break;
            }
        }
    });
...
// Función que se ejecutará cuando cambie la selección del tipo envio
...
function FinalizarCompra() {
    var radios = document.getElementsByName('optiones-envio');
    var factura = "· · ·     Factura Finalizacion De Compra     · · · ";
            ...
            switch (radio.id) {
                case "option1": // Domicilio
                    ...
                    for (const key of keys) {
                        ...
                        if(item == "Logeado") {
                            const strigDatosUsuarioLogeado = localStorage.getItem(key);
                            const datosUsuarioLogeado = JSON.parse(strigDatosUsuarioLogeado);
                            factura += ("\n· El envio se realizara a su domicilio: " + datosUsuarioLogeado.direccion);
                        }
                    }
                    break;
                case "option2": // Sucursal correo argentino
                    factura += ("\n· El envio se realizara a la sucursal mas cercana de correo argentino");
                    break;
                case "option3": // Nuestro Local
                    factura += ("\n· Podra pasar a retirar nuestros productos por nuestro local ubicado en: Shopping Alto Avellaneda, Gral. Güemes 897");
                    break;
            }
        }
    });
    ...
    factura += ("\n· Productos Comprados: ");
    for(var aux of JSON.parse(sessionStorage.getItem('listaCarrito'))) {
        for(var producto of listaProductos) {
            if(producto.id == aux.idProducto) {
                factura += ("\n  " + aux.cantidad + " - " + producto.nombre + " - $" + (producto.precio*aux.cantidad).toFixed(2));
                break;
            }
        }
    }

    factura += ("\n· El precio final de la compra es $" + (CalcularPrecioBrutoProductos() + CalcularPrecioEnvio()));
    factura += ("\n· · ·     Esta seguro de que desea realizar la compra?     · · · ");

    if(CalcularPrecioBrutoProductos() > 0) {
        confirm(factura);
    }
    else {
        alert("Debe agregar minimo un producto al carrito para poder realizar una compra...");
    }
}